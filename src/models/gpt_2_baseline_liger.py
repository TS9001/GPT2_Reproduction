...
class Block(nn.Module):
    def __init__(self, config: GPT2Configuration):
        super().__init__()
        self.config = config
        self.ln_1 = LigerLayerNorm(config.d_model) if config.use_liger else nn.LayerNorm(config.d_model)
        self.attn = Attention(config)
        self.ln_2 = LigerLayerNorm(config.d_model) if config.use_liger else nn.LayerNorm(config.d_model)
        self.mlp = MLP(config)
...
class GPT2Basic(nn.Module):
    def __init__(self, config: GPT2Configuration):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f=LigerLayerNorm(config.d_model) if config.use_liger else nn.LayerNorm(config.d_model)
        ))
